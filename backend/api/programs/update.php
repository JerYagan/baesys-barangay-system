<?php
/**
 * Baesys — Update Program/Project API (Admin/Staff only)
 * 
...
if (!$input || empty($input['id']) || empty($input['name']) || empty($input['status'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'ID, program name, and status are required fields.']);
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare('
        UPDATE programs 
        SET name = ?, description = ?, status = ?, start_date = ?, end_date = ?, budget = ?, target_beneficiaries = ?, updated_at = NOW() 
        WHERE id = ?
    ');
    $stmt->execute([
        $name,
    ...
    $logStmt->execute([
        ...
        'programs',
        $id,
        "Updated program/project '$name' (Status: $status)",
    ...
    echo json_encode(['success' => false, 'message' => 'Failed to update program details: ' . $e->getMessage()]);
}
